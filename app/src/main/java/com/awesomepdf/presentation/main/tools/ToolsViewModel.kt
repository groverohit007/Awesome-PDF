...
class ToolsViewModel @Inject constructor(
    ...
    fun startMerge(outputName: String = "merged_${System.currentTimeMillis()}.pdf") {
        if (_selectedUris.value.size < 2) {
            val message = "Please select at least 2 PDFs"
    ...
    private fun handleWorkInfo(info: WorkInfo) {
        ...
        when (info.state) {
            ...
            WorkInfo.State.FAILED -> {
                _isMerging.value = false
                val message = info.outputData.getString(MergePdfWorker.KEY_ERROR) ?: "Merge failed"
                _error.value = message
                crashlytics.recordException(IllegalStateException("Merge flow failed: $message"))
            }
